...
def get_label(file_path, class_names):
    ...
    Args:
        file_path: String -- Path of the input image
        class_names: list of Strings -- List of strings containing each class of the input folders
    Returns:
        label: String -- input images' label
    '''
...
def load_img(img, img_height, img_width, num_channels):
...
def process_img(file_path, class_names, img_height, img_width, num_channels):
    '''        Preprocess images
    Args:
        file_path: String -- Path of the input image
        class_names: list of Strings -- List of strings containing each class of the input folders
    ...
    # Read and convert images
    img = load_img(img, img_height, img_width, num_channels)
...
def data_preprocess(input_path, img_height, img_width, num_channels, batch_size, val_batch_size, cache):
    '''
    ...
    Args:
        ...
        cache (bool or str): caches the data if it doesn't fit in memory. use a `str` for big datasets
    Returns:
        ...
        validation_batches: Tensorflow.Dataset -- Batches of test images
    ...
    train = train_ds.map(lambda file_path: process_img(file_path, class_names, img_height, img_width, num_channels),
                         num_parallel_calls=AUTOTUNE)
    validation = val_ds.map(lambda file_path: process_img(file_path, class_names, img_height, img_width, num_channels),
                            num_parallel_calls=AUTOTUNE)

    train_batches = prepare_for_training(train, batch_size, cache)
...
def data_preprocess_test(test_path, img_height, img_width, num_channels, class_names):
    '''
        Prepare the dataset for training validation and test
    Args:
        test_path: String -- Path of the input image
        img_height: int -- Image height
        img_width: int -- Image width
        num_channels: int -- Number of channels on the images
        class_names:
    Returns:
        test_data: Tensorflow.Dataset -- Batches of test images
    '''
    test_dir = pathlib.Path(os.path.join(os.getcwd(), test_path))
    test_ds = tf.data.Dataset.list_files(str(test_dir / '*/*'))
    test_data = test_ds.map(lambda file_path: process_img(file_path, class_names, img_width, img_height, num_channels),
                            num_parallel_calls=AUTOTUNE)
